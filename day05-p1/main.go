...
func executeMove(d *data, m move) {
	...
	for i := 0; i < m.amount; i++ {
		// Pop Front "from" colummn
		crate, d.crate_map[m.from-1] = d.crate_map[m.from-1][0], d.crate_map[m.from-1][1:]
		// Push Front "to" column
		d.crate_map[m.to-1] = append([]string{crate}, d.crate_map[m.to-1]...)
	}
}
